...
<Note>
  Iris Code matches uppercase SQL keywords (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `DROP`) only. Error message strings that happen to contain the word "delete" in lowercase are not flagged. Use parameterised queries or a query builder that handles escaping for you.
</Note>
...
Applies to all four supported languages. Iris Code flags the literal regex pattern; move to a linear-time regex engine or rewrite the pattern to eliminate nested quantifiers.
...
<Note>
  Iris Code automatically skips test files for this pattern. A `localhost` URL in a `*.test.ts` or `*_test.go` file is not flagged.
</Note>
...
<Note>
  Iris Code skips test files for this pattern - `debug: true` in a test setup file is intentional.
</Note>
...
Run `iris security` as a standalone scan - it is free and requires no authentication:
...
<Tip>
  Run `iris security` as a free CI step even without a Pro licence - it catches these nine patterns on every push regardless of licence status, independent of a full `iris check` run.
</Tip>
...
Set `gateMaxSecuritySmells` in `.irisconfig.json` to fail the gate when the total number of security smells across the workspace exceeds a threshold - enforced by the CLI, the git pre-push hook, and the build hook, the same way `gateMaxSecrets` enforces a ceiling on hardcoded secrets.
...
<Tip>
  Set it to `0` to require a clean workspace, or leave it unset for no limit while you work through an existing backlog. See [Gate Limits](/configuration/irisconfig#gate-thresholds) for the full list of gate fields.
</Tip>
...
<Tip>
  Use `severityOverrides` to promote all nine security smell patterns to `error` severity at once if your team treats them as hard blockers rather than warnings: `"severityOverrides": { "security-smell": "error" }`. The override applies to the whole category - there is currently no way to override a single pattern (e.g. just SQL injection) independently of the other eight.
</Tip>
