...
`IRIS_SLACK_WEBHOOK` is the only place Iris Code reads the webhook, and it works with both `iris gate` and `iris check`. The URL is a posting credential, so Iris Code never accepts it as a command-line argument or reads it from `.irisconfig.json`; that keeps it out of shell history, process listings and committed files.
