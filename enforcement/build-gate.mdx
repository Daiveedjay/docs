...
<Steps>
  <Step title="Detects your project type">
    Looks for `package.json` (Node), `go.mod` (Go), `pyproject.toml` / `requirements.txt` (Python), or `Gemfile` (Ruby).
  </Step>
  <Step title="Installs the hook">
    ...
    - **Go, Python, and Ruby**: writes an `iris-check` target into your Makefile and wires it as a prerequisite of the `build` or `all` target.
  </Step>
