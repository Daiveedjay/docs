...
<Steps>
  <Step title="Detects your project type">
    Looks for `package.json` (Node), `go.mod` (Go), `pyproject.toml` / `requirements.txt` (Python), `Gemfile` (Ruby), or a `.csproj` / `.sln` (.NET).
  </Step>
  <Step title="Installs the hook">
    ...
    - **Go, Python, Ruby, and .NET**: writes an `iris-check` target into your Makefile and wires it as a prerequisite of the `build` or `all` target. A Makefile created for a .NET project defaults to `dotnet build`.
  </Step>
