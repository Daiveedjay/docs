...
<Steps>
  <Step title="Detects your project type">
    Looks for `package.json` (Node), `go.mod` (Go), or `pyproject.toml` / `requirements.txt` (Python).
  </Step>
  <Step title="Installs the hook">
    ...
    - **Go and Python**: writes an `iris-check` target into your Makefile and wires it as a prerequisite of the `build` or `all` target.
  </Step>
