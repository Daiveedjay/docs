...
entitled to run it - the licence server unreachable, or rate limiting a repeated caller -
so it stops rather than reporting a pass it has not earned. Treat it as retryable
infrastructure trouble rather than a code problem. A repository with no Pro licence is a
different case and still exits `0`.
