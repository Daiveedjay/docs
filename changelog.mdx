...
- **Added** ordered path/role overrides for Free file naming policies. A project can enforce `PascalCase` under component folders and `camelCase` under hooks or composables even when every file resolves to TypeScript. Workspace ignores remain absolute, the last matching rule wins, and existing language naming policies remain fallbacks.
