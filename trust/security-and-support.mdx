...
Iris Code uses different data boundaries for local analysis and opt-in Cloud audits. This page explains what each workflow reads, which network requests it makes, and how to report a security issue privately.
...
Cloud audits are different. When you explicitly connect a GitHub repository, the Cloud worker temporarily fetches its source, runs the audit, deletes the working copy, and retains the commit-pinned report. See [Cloud security and retention](/cloud/overview#security-and-retention) for that boundary.
