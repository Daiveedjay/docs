...
| `--json` | Output results as JSON |
| `--config <path>` | Path to `.irisconfig.json` |
| `--format github` | Emit inline PR annotations in GitHub Actions format |
...
**Pro** for install and uninstall; **free** for status. Installs and removes the git pre-push hook and the build gate hook — equivalent to the VS Code command palette hook commands.
