...
description: "Full reference for all twelve Iris CLI commands: iris check, iris secrets, iris security, iris deps, iris cve, iris sbom, iris todos, iris gate, iris report, iris hook, iris auth, and iris config."
---

The Iris CLI provides twelve commands covering the full code health workflow - scanning individual files, auditing dependencies, gating on CVEs, exporting SBOMs, enforcing thresholds in CI, and managing hooks. Commands marked **Pro** require an active Iris Pro licence.
...
**Pro.** Runs the same dependency and CVE scan as `iris deps` (sharing its 24-hour cache and network consent), but exits `1` only when a vulnerability at or above the `--severity` threshold is found. This is the command to use for CI gates: block merges on high or critical advisories without failing the pipeline on low-severity noise.
