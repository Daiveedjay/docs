...
| Flag | Description |
|---|---|
| `--root <path>` | Directory the server may read. Every requested path must resolve inside it, symlinks included (defaults to current directory) |
| `--sdk` | Serve the same tools through the official MCP TypeScript SDK instead of the built-in transport |

```bash
# Run the server for the current project
npx -y @iris-code/mcp --root .
