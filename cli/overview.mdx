---
title: "Iris Code CLI: Check and Enforce Project Standards from the Terminal"
sidebarTitle: "Overview"
description: "Use @iris-code/cli to run the same deterministic checks and gates from a terminal, git hook, build script, or CI pipeline."
---
...
- **`iris init`** - set a project up: asks three short questions, recommends a preset, writes `.irisconfig.json`, creates or refreshes the `AGENTS.md` rules block, and installs the pre-push hook where the licence allows
- **`iris fix --safe <file>`** - remove findings Iris Code can prove are safe to remove, on a single file; previews by default and never writes without confirmation
...
- **`iris rules`** - write this project's configured thresholds into the file your coding agent already reads, generated from `.irisconfig.json`
- **`iris mcp`** - run Iris Code as a Model Context Protocol server, so an agent can query real findings, thresholds and gate status
- **`iris auth login|status|logout`** - manage authentication credentials
...
- **`iris gate`** - the CI gate; exits `1` if any configured threshold is exceeded (`minHealthScore`, `gateMaxSecrets`, `gateMaxComplexity`, `gateMaxFileLength`, `gateMaxSmellsPerFile`, `gateMaxSecuritySmells`)
...
<CardGroup cols={2}>
  ...
  <Card title="Commands" icon="terminal" href="/cli/commands">
    Full reference for all seventeen Iris Code CLI commands, flags, and exit codes.
  </Card>
  <Card title="CI Integration" icon="gear" href="/cli/ci">
    Use `iris gate` as the gate in GitHub Actions, GitLab CI, or any shell pipeline.
  </Card>
