---
title: "Use Iris Code in GitHub Actions, GitLab CI, and shell pipelines"
...
Install the CLI, pass your licence as an environment variable, and run `iris gate` as your gate step.
...
| `3` | Your Iris Code licence could not be verified, so the gate did not run | Fail the build, or retry |

Exit `3` means the gate **did not run**, which is not the same as it finding nothing. It is
returned when a licence exists but could not be checked - the licence server unreachable, or
rate limiting a repeated caller. A gate that skipped silently and reported success would let a
build through that should have been blocked, so it fails closed instead. A user with no Pro
licence is a different case: that is known, so those commands skip and exit `0` and the
...
The message is built and sent by your own runner and never passes through Iris Code servers, so it can name the failing files. It stays quiet on a passing run unless you add `--slack-on always`, and a Slack outage can never change the exit code. See [Slack notifications](/enforcement/slack-notifications) for the full setup.
