...
Most CLI commands require an active Iris Pro licence. Authenticate via the interactive `iris auth login` flow for local development, or set the `IRIS_LICENCE_TOKEN` environment variable for non-interactive CI runners — no login step needed on the runner.
