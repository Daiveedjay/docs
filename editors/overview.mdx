...
`iris-lsp` is a standalone language server that speaks stdio JSON-RPC and carries the whole analysis engine, which is what JetBrains support is built on. In principle it works in Neovim, Helix, Sublime Text and anything else that speaks LSP.

In practice we do not yet publish the compiled binaries anywhere you can download them, so there is no supported setup path today. When that changes this page will carry the configuration snippets. We would rather say that plainly than publish a guide that depends on a download that does not exist.

## Reporting a problem

Because the same symptom usually has a different cause per host, please say which editor and which version you are using. The [bug report template](https://github.com/Daiveedjay/Iris-VSCode-Extension/issues/new/choose) asks for both.
