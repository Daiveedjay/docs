...
description: "Config Studio is a visual editor in your account dashboard. Pick a preset, tune thresholds, and sync the resulting config to VS Code, JetBrains IDEs, or any VS Code fork with a diff preview."
---

Config Studio is a visual editor at [iriscode.co/account/config](https://www.iriscode.co/account/config) - pick a preset, configure Free workspace, path/role, and fallback language naming, then sync the resulting `.irisconfig.json` to any supported editor. Pro adds custom thresholds, gate limits, and severity overrides.

## What the studio contains

Config Studio surfaces the team-policy configuration in one place, with a live JSON preview updating as you make changes:
...
<Warning>
  Four settings are deliberately not in the studio, because they describe one
  machine or one project rather than a shared standard: `ignoreFiles`,
  `ignoreFunctions`, `testConvention`, and `sidebarFontSize`.

  Syncing **replaces the whole `.irisconfig.json`**, so if your file already sets
  any of those four, they are removed. The diff shown before the write lists every
  removed line - read it, and re-add what you need afterwards. To keep them out of
  the sync path entirely, set them in
  [VS Code settings](/configuration/vscode-settings) instead, which sync never
  touches.
</Warning>
