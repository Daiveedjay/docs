...
Valid conventions are `snake_case`, `kebab-case`, `camelCase`, and `PascalCase`. Iris Code checks the file stem, preserves known suffixes such as `.test` and `.spec`, and suggests a replacement name.

## How Iris Code chooses a policy

For every file, Iris Code applies one deterministic precedence order:
...
- With `.iris-baseline.json`, Iris Code checks paths not present in the baseline. A renamed path counts as new.
- Without a baseline, Iris Code checks the whole scan. It shows the first 50 violations governed by `new-files` and discloses the full count; findings from a matching rule with `scope: "all"` are never hidden by that rollout cap.
