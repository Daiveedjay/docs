---
title: "Configure shared project standards with .irisconfig.json"
...
keywords: ["C#", "c sharp", "csharp", ".NET", "dotnet", ".csproj", "languages.csharp", "Java", "languages.java", ".properties", "Rust", "languages.rust", "Cargo", "per-language thresholds", "golang", "Gemfile"]
---

<Note>
  This page documents **configurable keys**, not a rule inventory. It mixes gate limits,
  thresholds, scoring weights, severity overrides and suppressible rule ids, so counting
  entries here does not give the number of rules Iris Code measures. For that, see the
  [accuracy benchmark](/trust/accuracy-benchmark), which publishes every measured rule
  per language.
</Note>
...
Generated configs show the active detection and gate choices with one-line comments. Team-specific items such as naming conventions, ignore lists, and language overrides remain commented examples until the project chooses them.
...
```json .irisconfig.json
{
  ...
  "gateMaxParseErrors": 0,     // Pro - max unparseable files (unset = report, never block)
...
|-----|---------|-----------------|
| `functionLengthThreshold` | `40` | Lines before a function is flagged. In `.jsx`/`.tsx`, JSX markup does not count toward this - a component is measured by its logic, the same way a Vue or Svelte `<template>` never counted. `fileLengthThreshold` still counts every line, so a large component is still flagged as a long file |
...
| `gateMaxParseErrors` | Pro | Maximum files Iris Code could not parse. **Unset by default, so an unreadable file is reported without failing the gate** - a real syntax error is already caught by your compiler and your build, where Iris Code adds nothing. Set to `0` to treat any unparseable file as a failure. Unparseable files are excluded from the workspace average either way, so they never drag your score down |
...
Scan-total gates (`minHealthScore`, `gateMaxSecrets`, `gateMaxSecuritySmells`, `gateMaxDuplicateBlocks`, `gateMaxNamingViolations`, `gateMaxParseErrors`), `presetId`, and `enable*` toggles are rejected inside a language block with a validation warning.
...
Unknown language keys, such as an `elixir` block written for a newer Iris Code release, produce a warning and are skipped, so a forward-looking config does not break an older installation. A per-file gate cap set only inside a language block still gates files of that language, even with no top-level cap configured.
