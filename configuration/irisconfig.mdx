...
## Comments and editor support

`.irisconfig.json` is JSONC, so `//` comments are valid. Every config Iris Code generates is commented, which is why a preset file explains each setting inline.

Generated configs also start with a `$schema` line:
...
Your editor resolves that itself, which gives you completion for every field and validation of values and enums in any editor with JSON Schema support - VS Code, JetBrains IDEs, Neovim - with no plugin-specific setup. Keep the line when you edit the file by hand.

**Hover any setting to see what it does.** Iris Code serves that documentation from the schema bundled with your installed version, so it works offline and behind a proxy, and it always matches the version of Iris Code you are running rather than whatever the remote schema happens to say.
...
Top-level `fileNaming` and `languages.*.fileNaming` are Free. The top-level policy applies to every supported language; a nested policy replaces it for that language. The six length/complexity thresholds, `healthScoreWeights`, `severityOverrides`, and per-file gate caps remain Pro. On Free, Iris applies the naming policy and places a warning squiggle on each Pro-only sibling field it ignores. It does not discard the whole language block.

Set top-level `fileNamingScope` to `"new-files"` (default) or `"all"`. A language can override that default with `languages.*.fileNaming.scope`; omit the nested scope to inherit the global value. See [File naming conventions](/configuration/file-naming) for baseline behavior, suggestions, ignore globs, and the complete exemption list.
