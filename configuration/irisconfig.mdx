...
```json .irisconfig.json
{
  ...
  "gateMaxDuplicateBlocks": 10, // Pro — max duplicate code blocks across the scan
...
| `duplicateBlockMinTokens` | `40` | Minimum normalised-token run length (identifiers, literals, whitespace, and comments ignored) before two stretches of code count as a duplicate block. Blocks where both sides are `.jsx`/`.tsx` files require twice this value - JSX markup is structurally repetitive, so small matches there are template echoes rather than copy-paste debt |
...
| `gateMaxDuplicateBlocks` | Pro | Max duplicate code blocks allowed across the whole scan (a block spanning two files counts once). Unset on every preset until you configure it |
