...
description: "Iris natively parses and scores TypeScript, JavaScript (.js/.jsx/.ts/.tsx), Go, and Python with language-specific function detection, import tracking, and debug-print flagging."
---
...
<Tabs>
  <Tab title="TypeScript & JavaScript">
    Iris scans `.js`, `.jsx`, `.ts`, and `.tsx` files with a single TS/JS parser. All four extensions are treated as the same language family and share the same rule set.
  ...
  <Tab title="Go">
    Iris parses `.go` files with a Go-specific analyser. Language-aware behaviours include:
    ...
    - **No-exports warning** — Iris raises a warning if no capitalised (exported) function names are found in the file
  ...
  <Tab title="Python">
    Iris parses `.py` files with a Python-specific analyser. Language-aware behaviours include:
    ...
    - **No public functions warning** — Iris raises a warning if every function in the file starts with `_` (private by convention), indicating no public API is exposed
