---
title: "Supported Languages: TypeScript, JavaScript, Vue, Svelte, Go, Python, Ruby, C#"
sidebarTitle: "Language Support"
description: "Iris Code natively parses and scores TypeScript, JavaScript, Vue, Svelte, Go, Python, Ruby, and C# with per-language function detection, import tracking, and debug-print flagging."
keywords: ["C#", "c sharp", "csharp", "c-sharp", ".NET", "dotnet", "dotnet core", "ASP.NET", "ASP.NET Core", ".cs", ".csproj", "MSBuild", "NuGet", "LINQ", "async void", "EF Core", "Entity Framework", "xUnit", "NUnit", "MSTest", "golang", "Rails", "ERB", "Gemfile", "single-file component", "SFC", "TSX", "JSX"]
---

Six languages, each with its own parser, plus Vue and Svelte components whose script blocks go through the JavaScript or TypeScript one.
...
<Note>
  **Don't compare scores across languages.** A TypeScript file carries deductions that simply don't exist in Go, Python, Ruby, or C#: `any`, `@ts-ignore`, missing return types, non-null assertions. An 80 in Go and an 80 in TypeScript are not the same 80, and ranking a mixed codebase by score across languages will mislead you.
</Note>
...
<Tabs>
  ...
  <Tab title="Ruby">
    ...
    **Language-aware behaviours:**
    ...
    <Note>
      **No unused-code detection in Ruby.** Ruby resolves methods and constants at runtime, so a constant referenced only through `send`, `const_get`, or a Rails autoload is invisible to static analysis. Iris Code reports no unused imports or unused gems for Ruby rather than reporting ones it cannot stand behind. Unused dependency detection covers npm, Go, and Python manifests only.
    </Note>
  </Tab>
  ...
  <Tab title="C#">
    ...
    **Language-aware behaviours:**
    ...
    <Note>
      **No unused-code detection and no NuGet advisories in C#.** Dependency injection, reflection, partial classes and source generators all reference code in ways static analysis cannot see, so Iris Code reports no unused members or unused packages for C# rather than reporting ones it cannot stand behind. A `.csproj`'s packages count towards a file's dependency signal, but no version or vulnerability lookup runs for them: the [dependency table](/features/dependents-table) covers npm, Go and Python manifests only.
    </Note>
  </Tab>
</Tabs>
