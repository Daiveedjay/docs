---
title: "Dependents Table: Audit npm, Go, Python, RubyGems, and NuGet Packages"
...
The Dependents Table audits every third-party package in your project: the version you have installed against the latest available, and any known vulnerabilities from the [OSV.dev](https://osv.dev) database. npm, Go modules, Python, RubyGems, and NuGet appear side by side, instead of in separate tools with separate output formats.
...
Advisories are then filtered to the version you have installed. A CVE that only affects 3.x is not reported when you are on 4.x, which keeps the list to things you can act on. RubyGems lockfiles include the full resolved tree; NuGet without `packages.lock.json` scans direct package declarations only. Unpinned or conflicting versions are marked **not checked**, never assumed clean.
