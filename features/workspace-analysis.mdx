---
title: "Workspace analysis: find project-wide risks and trends"
sidebarTitle: "Workspace Analysis"
description: "Review supported files across a workspace or folder, rank hotspots, inspect findings and TODOs, and track changes over time."
---
...
Your **first full workspace scan is free** and needs no account. Iris Code saves it locally, so the rankings, issues, TODOs, security findings and duplicates stay available without scanning again. Rescanning requires Pro.

The saved result also powers [Code Map](/features/code-map), a tree view in the Code Map panel that shows where the measured health scores sit in the repository.

## Running one

**Iris Code: Scan Workspace** from the command palette, or the **Scan Workspace** button in the Workspace tab. It walks every supported file and gives you:
...
Right-click any folder in the Explorer and pick **Iris Code: Scan This Folder**. Or hit **Scan Current Folder** in the Folder tab to scan whatever the open file lives in.

The **Folder** tab shows the same stats, chart, rankings and tree as the workspace, with its own Rescan and Clear.

<Note>
  One full workspace scan is free without an account. Rescanning and folder scans are Pro. File reviews and click-to-open stay free forever.
</Note>
