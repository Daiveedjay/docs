...
keywords: ["C#", "c sharp", "csharp", ".NET", "dotnet", ".cs", "method detection", "unused imports", "language badge"]
---
...
| **Language badge** | Go, TypeScript, JavaScript, Python, Ruby or C#, in the header |
...
Four additional signals appear in `.ts`, `.tsx`, `.js` and `.jsx` files. They are hidden entirely in Go, Python, Ruby, and C# rather than shown as empty rows.
...
<Note>
  Unused *import* detection is TypeScript and JavaScript only. Go and Python handle unused imports at the compiler or runtime level, so Iris Code does not duplicate that, Ruby resolves constants at runtime, where static analysis cannot tell an unused require from one loaded through `send` or Rails autoloading, and C# resolves types through dependency injection, reflection, and source generators for the same reason.
</Note>
