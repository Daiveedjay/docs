...
<Note>
  The table on this page lists the **weights used to compute a health score**, not the
  set of rules Iris Code measures. Several rules carry no score weight, and several
  weights cover a group of findings rather than one rule. The measured-rule count, per
  language, is on the [accuracy benchmark](/trust/accuracy-benchmark) page.
</Note>
